...
Now we have a File with the Tweets (step3) and one which shows the language for each word (step2) Both contain the tweetID
...
If necessary will this script add the column names to the files.
...
Languages gives the languages for every word. Which language the word has is given in the "offsetFile.tsv" (via label and start stop markers). 
...
The scirpt picks the TweetID from the StringFile. Looks up every corresponding row in the OffSetFile. Create an tweet and language array. Those will be put in the table.
That way has the script for every tweetID the string, word-array and the language-array. 
...
Stepts 1-5 are necessary to perform for the trainings-, trial-, and test-data

6. 
use the script unigramsWordLevel.py this will create all unigrams from the StringFile
Those are later typed manualy into python scripts since otherwise encoding errors occur.
